...
export class InMemoryUserGateway implements UserGateway {
  private readonly _mockData: UserData = {
    workspaces: [
      {
        ...
        statuses: [
          {
            id: "status-todo-bernard",
            name: "À faire",
            color: "#E3F2FD",
            workspaceId: "09e70007-8630-4948-a234-d8d020c5de6e",
            createdAt: "2026-01-11T15:21:41.800+00:00",
            updatedAt: null,
          },
          {
            id: "status-in-progress-bernard",
            name: "En cours",
            color: "#FFF3E0",
            workspaceId: "09e70007-8630-4948-a234-d8d020c5de6e",
            createdAt: "2026-01-11T15:21:41.800+00:00",
            ...
            tasks: [
              {
                ...
                statusId: "status-in-progress-bernard",
      ...
      },
      {
        ...
        statuses: [
          {
            id: "status-todo-leclerc",
            name: "À faire",
            color: "#E3F2FD",
            workspaceId: "1ad0d56c-73ef-4b5b-89bb-42e17028a6b9",
            createdAt: "2026-01-12T18:14:06.470+00:00",
            updatedAt: null,
          },
          {
            id: "status-in-progress-leclerc",
            name: "En cours",
            color: "#FFF3E0",
            workspaceId: "1ad0d56c-73ef-4b5b-89bb-42e17028a6b9",
            createdAt: "2026-01-12T18:14:06.470+00:00",
            ...
            tasks: [
              {
                ...
                statusId: "status-in-progress-leclerc",
  ...
  getUserData(): Observable<UserData> {
    return defer(() => new BehaviorSubject(this._mockData).asObservable());
  }
  ...
  createWorkspace(dto: CreateWorkspaceDto): Observable<Workspace> {
    ...
    this._mockData.workspaces.push(newWorkspace);
    return defer(() => new BehaviorSubject(newWorkspace).asObservable());
  }

  createColumn(dto: CreateColumnDto): Observable<Column> {
    const workspace = this._mockData.workspaces.find((w) => w.id === dto.workspaceId);
    if (!workspace) {
      throw new Error(`Workspace ${dto.workspaceId} introuvable`);
    }
    ...
    const newColumn: Column = {
      ...
      position: workspace.columns.length,
      createdAt: new Date().toISOString(),
      updatedAt: null,
      tasks: [],
    };

    workspace.columns.push(newColumn);
    return defer(() => new BehaviorSubject(newColumn).asObservable());
  }

  reorderColumn(columnId: string, dto: ReorderColumnDto): Observable<Column> {
    // Trouver le workspace contenant cette colonne
    const workspace = this._mockData.workspaces.find((w) =>
      w.columns.some((c) => c.id === columnId)
    );

    if (!workspace) {
      throw new Error(`Column ${columnId} introuvable`);
    }

    const columnIndex = workspace.columns.findIndex((c) => c.id === columnId);
    const [column] = workspace.columns.splice(columnIndex, 1);
    workspace.columns.splice(dto.newPosition, 0, column);

    // Mettre à jour les positions
    workspace.columns.forEach((col, index) => {
      col.position = index;
    });

    return defer(() => new BehaviorSubject(column).asObservable());
  }

  createTask(task: CreateTaskDto): Observable<Task> {
    // Trouver le workspace et la colonne
    const workspace = this._mockData.workspaces.find((w) =>
      w.columns.some((c) => c.id === task.columnId)
    );

    if (!workspace) {
      throw new Error(`Column ${task.columnId} introuvable`);
    }
    ...
    return defer(() => new BehaviorSubject(newTask).asObservable());
  }

  updateTask(task: Task): Observable<Task> {
    // Trouver le workspace et la colonne
    const workspace = this._mockData.workspaces.find((w) =>
      w.columns.some((c) => c.tasks.some((t) => t.id === task.id))
    );

    if (!workspace) {
      throw new Error(`Task ${task.id} introuvable`);
    }

    const column = workspace.columns.find((c) => c.tasks.some((t) => t.id === task.id));
    if (!column) {
      throw new Error(`Task ${task.id} introuvable`);
    }
    ...
    return defer(() => new BehaviorSubject(column.tasks[taskIndex]).asObservable());
  }

  reorderTask(taskId: string, dto: ReorderTaskDto): Observable<Task> {
    // Trouver le workspace et la colonne source
    const workspace = this._mockData.workspaces.find((w) =>
      w.columns.some((c) => c.tasks.some((t) => t.id === taskId))
    );
    ...
    const sourceColumn = workspace.columns.find((c) => c.tasks.some((t) => t.id === taskId));
    if (!sourceColumn) {
      throw new Error(`Task ${taskId} introuvable`);
    }
    ...
    if (dto.newColumnId && dto.newColumnId !== task.columnId) {
      const targetColumn = workspace.columns.find((c) => c.id === dto.newColumnId);
      if (!targetColumn) {
        throw new Error(`Column ${dto.newColumnId} introuvable`);
      }

      task.columnId = dto.newColumnId;
      targetColumn.tasks.splice(dto.newOrder, 0, task);

      // Réordonner les tâches de la colonne cible
      targetColumn.tasks.forEach((t, index) => {
        t.order = index;
      });

      // Réordonner les tâches de la colonne source
      sourceColumn.tasks.forEach((t, index) => {
        t.order = index;
      });
    } else {
      // Même colonne, juste réordonner
      sourceColumn.tasks.splice(dto.newOrder, 0, task);
      sourceColumn.tasks.forEach((t, index) => {
        t.order = index;
      });
    }
    ...
    return defer(() => new BehaviorSubject(task).asObservable());
  }
  ...
  createStatus(dto: CreateStatusDto): Observable<Status> {
    ...
    workspace.statuses.push(newStatus);
    return defer(() => new BehaviorSubject(newStatus).asObservable());
  }

  updateStatus(statusId: string, dto: UpdateStatusDto): Observable<Status> {
    const workspace = this._mockData.workspaces.find((w) =>
      w.statuses.some((s) => s.id === statusId)
    );

    if (!workspace) {
      throw new Error(`Status ${statusId} introuvable`);
    }

    const statusIndex = workspace.statuses.findIndex((s) => s.id === statusId);
    const updatedStatus: Status = {
      ...workspace.statuses[statusIndex],
      ...(dto.name && { name: dto.name }),
      ...(dto.color && { color: dto.color }),
      updatedAt: new Date().toISOString(),
    };

    workspace.statuses[statusIndex] = updatedStatus;
    return defer(() => new BehaviorSubject(updatedStatus).asObservable());
  }
  ...
  deleteStatus(statusId: string): Observable<void> {
    ...
    const statusIndex = workspace.statuses.findIndex((s) => s.id === statusId);
    workspace.statuses.splice(statusIndex, 1);

    return defer(() => new BehaviorSubject<void>(undefined).asObservable());
  }
}
